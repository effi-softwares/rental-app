...
const uploadTokenPayloadSchema = mediaClientPayloadSchema.extend({
...
function resolveUploadBucket(body: HandleUploadBody) {
	...
	if (body.type === "blob.upload-completed") {
		...
		const parsed = uploadTokenPayloadSchema.safeParse(parsedTokenPayload)
		...
		const result = await adapter.handleClientUpload({
			...
			onBeforeGenerateToken: async ({ pathname, clientPayload }) => {
				...
				if (payload.link.entityId !== rentalId) {
					throw new Error("Upload target does not match this rental.")
				}
				...
				if (
					payload.branchId &&
					payload.branchId !== scopedRental.record.branchId
				) {
					throw new Error("Upload branch does not match the rental branch.")
				}
				...
				const tokenPayload = uploadTokenPayloadSchema.parse({
					...payload,
			...
			onUploadCompleted: async ({ blob, tokenPayload }) => {
				const parsedTokenPayload = uploadTokenPayloadSchema.safeParse(
